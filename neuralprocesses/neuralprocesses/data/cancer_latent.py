...
class CancerLatentGenerator(DataGenerator):
    ...
    def __init__(
        ...
        # reshape and scale the observations, in 2D the difference and the latent variable acid
        max_value = 10
        self.trajectories1=[(tr[0]-tr[1]).reshape(ntime, nx, nx)/max_value for tr in trajectories_data]
    ...
    def generate_batch(self):
        with B.on_device(self.device):
            ...
            for b in range(self.batch_size):
                ...
                #print(type(x))
                y = B.concat(self.trajectories1[inds[b]][time, x1, x2].reshape(1,-1),self.trajectories2[inds[b]][time, x1, x2].reshape(1,-1))
                #print(type(y))
                context_x[b] = x
